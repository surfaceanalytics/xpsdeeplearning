...
os.chdir(os.path.join(os.path.abspath(__file__).split("deepxps")[0],
                      "deepxps"))
cw = os.getcwd()

# noqa: E402
from xpsdeeplearning.simulation.base_model.spectra import (
    MeasuredSpectrum, SimulatedSpectrum)
...
fig = plt.figure(figsize=(35, 9), dpi=30)
outer = gridspec.GridSpec(1, 3, wspace=0.2, hspace=3)
inner = gridspec.GridSpecFromSubplotSpec(
    2, 2,
    subplot_spec=outer[1], wspace=0.4, hspace=0.45)
...
ax0_0.set_xlabel("Binding energy (eV)",
              fontdict=fontdict)
ax0_0.set_ylabel("Intensity (arb. units)",
              fontdict=fontdict)
...
ax0_1.set_xlabel("Binding energy (eV)",
              fontdict=fontdict)
ax0_1.set_ylabel("Intensity (arb. units)",
              fontdict=fontdict)
...
ax1_0.set_xlabel("Binding energy (eV)",
              fontdict=fontdict)
ax1_0.set_ylabel("Intensity (arb. units)",
              fontdict=fontdict)
...
ax1_1.set_xlabel("Binding energy (eV)",
              fontdict=fontdict)
ax1_1.set_ylabel("Intensity (arb. units)",
              fontdict=fontdict)
...
for sim_values in sim_params.values():
    ...
    ax2.plot(sim.output_spectrum.x, sim.output_spectrum.lineshape, c=next(colors))
    ...
    ax2.set_xlim(left=np.max(sim.output_spectrum.x),
