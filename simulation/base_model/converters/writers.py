...
class TextWriter():
    ...
    def build_lines(self,data):
        ...
        for data in data:
            ...
            data_lines = [str(np.round(x,3)) + " " + str(y) for x,y in zip(data["data"]["x"], data["data"]["y0"])]
...
class VamasWriter():
    ...
    def write(self, data, filename):
        ...
        for spec in data:                
            ...
            if self.normalize != 0:
                norm = self.normalize
                y = [spec['data']['y0'][i] / spec['data']['y'+str(norm)][i]
                    for i in range(len(spec['data']['y0']))]                                               
            x_units = setting['x_units']
            if ((x_units == 'Binding Energy') & (setting['scan_mode'] != 'FixedEnergies')):
                block.abscissaStart = str(float(block.sourceEnergy) - float(setting['binding_energy']))
            else:
